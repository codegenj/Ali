...
final class PirParser implements ParserInterface
{
    public function parse(string $domain, string $tld, string $whoisServer, string $whoisRaw, string $notFoundPattern): DomainInfo
    {
        ...
        $info->createdAt = $this->parseDate($whoisRaw, '/Created On:\s*(.+)/i')
            ?? $this->parseDate($whoisRaw, '/Creation Date:\s*(.+)/i');
        $info->updatedAt = $this->parseDate($whoisRaw, '/Last Updated On:\s*(.+)/i')
            ?? $this->parseDate($whoisRaw, '/Updated Date:\s*(.+)/i');
